...
namespace SvgControl
{
	public class Shape : ClipArtElement
	{
		...
		protected virtual void Parse(SVG svg, string name, string value)
		{
			...
			{
				GetStroke(svg).LineCap = (SvgControl.Stroke.eLineCap)Enum.Parse(typeof(SvgControl.Stroke.eLineCap), value);
				return;
			}
			if (name == SVGTags.sStrokeLinejoin)
			{
				GetStroke(svg).LineJoin= (SvgControl.Stroke.eLineJoin)Enum.Parse(typeof(SvgControl.Stroke.eLineJoin), value);
				return;
			}
