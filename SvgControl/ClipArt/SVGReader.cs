...
namespace SvgControl
{
	...
	public class SVGRender
	{
		...
		GeometryDrawing NewDrawingItem(SvgControl.Shape shape, Geometry geometry)
		{
		...
		DrawingGroup LoadGroup(IList<Shape> elements)
		{
			...
			foreach (SvgControl.Shape shape in elements)
			{
				if (shape is SvgControl.UseShape)
				{
					SvgControl.UseShape useshape = shape as SvgControl.UseShape;
					SvgControl.Group group = SVG.GetShape(useshape.hRef) as SvgControl.Group;
					if (group != null)
					{
				...
				if (shape is SvgControl.Group)
				{
					DrawingGroup subgroup = LoadGroup((shape as SvgControl.Group).Elements);
					if (shape.Transform != null)
						subgroup.Transform = shape.Transform;
					grp.Children.Add(subgroup);
					continue;
				}
				if (shape is SvgControl.RectangleShape)
				{
					SvgControl.RectangleShape r = shape as SvgControl.RectangleShape;
				...
				if (shape is SvgControl.LineShape)
				{
					SvgControl.LineShape r = shape as SvgControl.LineShape;
					LineGeometry line = new LineGeometry(r.P1, r.P2);
					grp.Children.Add(NewDrawingItem(shape, line));
				}
				if (shape is SvgControl.PolylineShape)
				{
					SvgControl.PolylineShape r = shape as SvgControl.PolylineShape;
				...
				if (shape is SvgControl.PolygonShape)
				{
					SvgControl.PolygonShape r = shape as SvgControl.PolygonShape;
					PathGeometry path = new PathGeometry();
					PathFigure p = new PathFigure();
					path.Figures.Add(p);
					p.IsClosed = true;
				...
				if (shape is SvgControl.CircleShape)
				{
					SvgControl.CircleShape r = shape as SvgControl.CircleShape;
					EllipseGeometry c = new EllipseGeometry(new Point(r.CX, r.CY), r.R, r.R);
					grp.Children.Add(NewDrawingItem(shape, c));
				}
				if (shape is SvgControl.EllipseShape)
				{
					SvgControl.EllipseShape r = shape as SvgControl.EllipseShape;
					EllipseGeometry c = new EllipseGeometry(new Point(r.CX, r.CY), r.RX, r.RY);
					grp.Children.Add(NewDrawingItem(shape, c));
				}
				if (shape is SvgControl.ImageShape)
				{
					SvgControl.ImageShape image = shape as SvgControl.ImageShape;
					ImageDrawing i = new ImageDrawing(image.ImageSource, new Rect(image.X, image.Y, image.Width, image.Height));
					grp.Children.Add(i);
				}
				if (shape is SvgControl.TextShape)
				{
					GeometryGroup gp = TextRender.BuildTextGeometry(shape as SvgControl.TextShape);
					if (gp != null)
					{
				...
				if (shape is SvgControl.PathShape)
				{
					SvgControl.PathShape r = shape as SvgControl.PathShape;
					PathFigure p = null;
					Point lastPoint = new Point(0,0);

					SvgControl.PathShape.CurveTo lastc = null;
					Point lastcirPoint = new Point(0,0);

					PathGeometry path = new PathGeometry();
					foreach (SvgControl.PathShape.PathElement element in r.Elements)
					{
						bool isRelative = element.IsRelative;
						if (element is SvgControl.PathShape.MoveTo)
						{
							p = new PathFigure();
							p.IsClosed = r.ClosePath;
							if (isRelative)
								p.StartPoint = lastPoint + (Vector)((SvgControl.PathShape.MoveTo)element).Point;
							else
								p.StartPoint = ((SvgControl.PathShape.MoveTo)element).Point;
							lastPoint = p.StartPoint;
							path.Figures.Add(p);
							continue;
						}
						if (element is SvgControl.PathShape.LineTo)
						{
							SvgControl.PathShape.LineTo lineto = element as SvgControl.PathShape.LineTo;
						...
						if (element is SvgControl.PathShape.CurveTo)
						{
							SvgControl.PathShape.CurveTo c = element as SvgControl.PathShape.CurveTo;
						...
						if (element is SvgControl.PathShape.EllipticalArcTo)
						{
							SvgControl.PathShape.EllipticalArcTo c = element as SvgControl.PathShape.EllipticalArcTo;
